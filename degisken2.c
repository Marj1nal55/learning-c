...
float c =  3.141592; // 4 bytes 6-7 basamaklı değerler %f
double d = 3.1415926358973; //8 bytes 15-16 basamaklı değerler %lf

bool e = false; // 1 byte (true veya false) %d

char f = 120;  // 1 byte (-128 <-> +127) %d veya %c
unsigned char g = 255; // 1byte (0 <-> +255) %d veya %c (%d = sayı %c = denk gelen harf veya sembol)
...
unsigned long long int m = 18446744073709551615U; // 8 bytes (0 <-> 18 quintillion) %llu
 ...
 printf("%0.15lf\n", d);// double - ondalık sayılar
 printf("%d\n", e); // bool - true veya false
 printf("%c\n", f); // char - karakterin sayısal değeri
 printf("%d\n", g); // unsigned char - pozitif tek karakterin sayısal değeri
 printf("%d\n", h); // short - tam sayılar
 printf("%d\n", i); // unsigned short - pozitif tam sayılar
 printf("%d\n",j); // int - tam sayılar
